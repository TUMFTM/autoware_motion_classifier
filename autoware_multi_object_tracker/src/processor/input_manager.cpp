  ...
  const autoware_perception_msgs::msg::DetectedObjects::ConstSharedPtr msg)
{
  ...
  objects_que_.push_back(std::make_pair(std::move(objects_with_uncertainty), objects));
  ...
  ObjectsList & objects_list)
{
  ...
  for (const auto & objects : objects_que_) {
    const rclcpp::Time object_time = rclcpp::Time(objects.first.header.stamp);
    ...
    if (object_time <= object_latest_time) {
      std::pair<uint, std::pair<DetectedObjects, DetectedObjects>> objects_pair(index_, objects);
...
bool InputManager::getObjects(const rclcpp::Time & now, ObjectsList & objects_list)
{
  ...
  std::sort(
    ...
    [](
      const std::pair<uint, std::pair<DetectedObjects, DetectedObjects>> & a,
      const std::pair<uint, std::pair<DetectedObjects, DetectedObjects>> & b) {
      return (rclcpp::Time(a.second.first.header.stamp) - rclcpp::Time(b.second.first.header.stamp))
               .seconds() < 0;
    });
  ...
  if (is_any_object) {
    latest_exported_object_time_ = rclcpp::Time(objects_list.back().second.first.header.stamp);
  } else {
