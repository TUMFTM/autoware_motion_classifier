  ...
  last_updated_time_(this->now())
{
  ...
  /// Motion classifier
  /// TODO: add parameters for motion classifier
  {
    use_motion_classifier_ = declare_parameter<bool>("consider_statistical_motion");
    log_z_scores_ = declare_parameter<bool>("log_z_scores", false);
    const auto observation_window_size = this->declare_parameter<int>("observation_window_size");

    // Declare per-class z-score thresholds
    std::map<std::uint8_t, double> z_score_thresholds;
    z_score_thresholds[Label::UNKNOWN] =
      this->declare_parameter<double>("z_score_threshold.unknown", 1.96);
    z_score_thresholds[Label::CAR] = this->declare_parameter<double>("z_score_threshold.car", 1.96);
    z_score_thresholds[Label::TRUCK] =
      this->declare_parameter<double>("z_score_threshold.truck", 1.96);
    z_score_thresholds[Label::BUS] = this->declare_parameter<double>("z_score_threshold.bus", 1.96);
    z_score_thresholds[Label::TRAILER] =
      this->declare_parameter<double>("z_score_threshold.trailer", 1.96);
    z_score_thresholds[Label::MOTORCYCLE] =
      this->declare_parameter<double>("z_score_threshold.motorcycle", 1.96);
    z_score_thresholds[Label::BICYCLE] =
      this->declare_parameter<double>("z_score_threshold.bicycle", 1.96);
    z_score_thresholds[Label::PEDESTRIAN] =
      this->declare_parameter<double>("z_score_threshold.pedestrian", 1.96);

    const auto variance_lower_bound = this->declare_parameter<double>("variance_lower_bound");
    const size_t minimum_window_size =
      this->declare_parameter<int>("minimum_window_size_for_z_test");
    motion_classifier_ = std::make_unique<uncertainty::ObstacleMotionClassifier>(
      observation_window_size, z_score_thresholds, variance_lower_bound, minimum_window_size,
      log_z_scores_);
  }
...
void MultiObjectTracker::onTrigger()
{
  ...
  const rclcpp::Time latest_time(objects_list.back().second.first.header.stamp);
  ...
  if (!publish_timer_) {
    const auto latest_object_time = rclcpp::Time(objects_list.back().second.first.header.stamp);
...
/**
 * @brief Process input detected objects through the multi-object tracking pipeline.
 *
 * This function handles the complete tracking workflow including:
 * - Transforming input objects to world frame
 * - Handling odometry uncertainty (if enabled)
 * - Performing prediction, association, and update steps
 * - Managing tracker pruning and spawning
 * - Updating motion classification (if enabled)
 *
 * @param input_objects Pair of detected objects (first: transformed, second: raw)
 * @param channel_index Index of the input channel for tracking
 */
void MultiObjectTracker::runProcess(
  const std::pair<DetectedObjects, DetectedObjects> & input_objects, const uint & channel_index)
{
  // Get the time of the measurement
  const rclcpp::Time measurement_time =
    rclcpp::Time(input_objects.first.header.stamp, this->now().get_clock_type());
  ...
  DetectedObjects tracker_detections;
  DetectedObjects motion_classifier_objects;
  if (!autoware::object_recognition_utils::transformObjects(
        input_objects.first, world_frame_id_, tf_buffer_, tracker_detections)) {
    return;
  }
  if (!autoware::object_recognition_utils::transformObjects(
        input_objects.second, world_frame_id_, tf_buffer_, motion_classifier_objects)) {
    return;
  }
  // if (!tracker_detections.objects.empty() &&  motion_classifier_objects.objects.empty()) {
  //   // const auto & first_transformed_object = tracker_detections.objects.front();
  //   // const auto & first_raw_object = motion_classifier_objects.objects.front();

  //   // Example usage: log the first object's position
  //   // RCLCPP_WARN(
  //   //   rclcpp::get_logger("multi_object_tracker"),
  //   //   "transformed object: cov_x=%f, cov_y=%f",
  //   //   first_transformed_object.kinematics.pose_with_covariance.covariance[0],
  //   //   first_transformed_object.kinematics.pose_with_covariance.covariance[7]);

  //   // RCLCPP_WARN(
  //   //   rclcpp::get_logger("multi_object_tracker"),
  //   //   "raw object: cov_x=%f, cov_y=%f",
  //   //   first_raw_object.kinematics.pose_with_covariance.covariance[0],
  //   //   first_raw_object.kinematics.pose_with_covariance.covariance[7]);
  // }
  // RCLCPP_WARN(rclcpp::get_logger("multi_object_tracker"), "t=%ld, Static obj %s, vx=%f, vy=%f",
  //   time_.nanoseconds(), uuid.c_str(),
  //   object_test.kinematics.twist_with_covariance.twist.linear.x,
  //   object_test.kinematics.twist_with_covariance.twist.linear.y);
  ...
  if (enable_odometry_uncertainty_) {
    ...
    uncertainty::addOdometryUncertainty(odometry, tracker_detections);
  }
  // Normalize the object uncertainty
  uncertainty::normalizeUncertainty(tracker_detections);
  ...
  std::unordered_map<int, int> direct_assignment, reverse_assignment;
  {
    ...
    const auto & detected_objects = tracker_detections;
    ...
    debugger_->collectObjectInfo(
      measurement_time, processor_->getListTracker(), channel_index, tracker_detections,
      direct_assignment, reverse_assignment);
  }

  /* tracker update */
  processor_->update(tracker_detections, *self_transform, direct_assignment, channel_index);

  if (use_motion_classifier_) {
    motion_classifier_->update_observations(
      measurement_time, motion_classifier_objects, reverse_assignment,
      processor_->getListTracker());
  }
  /* tracker pruning */
  ...
  if (input_manager_->isChannelSpawnEnabled(channel_index)) {
    processor_->spawn(tracker_detections, *self_transform, reverse_assignment, channel_index);
  }
  /* object motion classification, second update to include newly spawned or removed trackers*/
  if (use_motion_classifier_) {
    motion_classifier_->update_observations(
      measurement_time, motion_classifier_objects, reverse_assignment,
      processor_->getListTracker());
...
void MultiObjectTracker::publish(const rclcpp::Time & time) const
{
  ...
  // Get objects for movement classification
  TrackedObjects output_msg, tentative_objects_msg;
  output_msg.header.frame_id = world_frame_id_;
  processor_->getTrackedObjects(time, output_msg);
  // Classify object motion state and set static state in trackers
  if (use_motion_classifier_) {
    motion_classifier_->determine_object_motion_state(
      time, processor_->getListTracker(), output_msg);
    // Get updated objects and publish
    processor_->getTrackedObjects(time, output_msg);
  }
