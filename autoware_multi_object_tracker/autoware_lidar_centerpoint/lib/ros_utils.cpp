  ...
  const bool has_variance, autoware_perception_msgs::msg::DetectedObject & obj)
{
  ...
  if (has_twist) {
    ...
    obj.kinematics.twist_with_covariance.covariance[XYZRPY_COV_IDX::X_X] = box3d.vel_x_variance;
    obj.kinematics.twist_with_covariance.covariance[XYZRPY_COV_IDX::Y_Y] = box3d.vel_y_variance;
  }
  /// TODO: if has_position_covariance is set to true, the uncertainties are passed to the kalman
  /// filter as is. This makes the tracking more unreliable (at least empirically).
