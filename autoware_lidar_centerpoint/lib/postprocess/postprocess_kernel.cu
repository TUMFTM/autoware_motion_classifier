...
{
  is_score_greater(const float * thresholds) : thresholds_(thresholds) {}

  __device__ bool operator()(const Box3D & b) { return b.score > thresholds_[b.label]; }

private:
  const float * thresholds_;
};
...
__global__ void generateBoxes3D_kernel(
  ...
  const float * out_rot, const float * out_vel, const float * out_offset_uncertainty,
  const float * out_rot_uncertainty, const float * out_vel_uncertainty, const float voxel_size_x,
  const float voxel_size_y, const float range_min_x, const float range_min_y,
  const std::size_t down_grid_size_x, const std::size_t down_grid_size_y,
  const std::size_t downsample_factor, const int class_size, const bool has_variance,
  const float * yaw_norm_thresholds, Box3D * det_boxes3d)
{
  ...
  det_boxes3d[idx].x_variance =
    expf(expf(out_offset_uncertainty[down_grid_size * 0 + idx]) - 10.0f);
  det_boxes3d[idx].y_variance =
    expf(expf(out_offset_uncertainty[down_grid_size * 1 + idx]) - 10.0f);
  det_boxes3d[idx].yaw_variance = expf(expf(out_rot_uncertainty[down_grid_size * 0 + idx]) - 10.0f);
  det_boxes3d[idx].vel_x_variance =
    expf(expf(out_vel_uncertainty[down_grid_size * 0 + idx]) - 10.0f);
  det_boxes3d[idx].vel_y_variance =
    expf(expf(out_vel_uncertainty[down_grid_size * 1 + idx]) - 10.0f);
}

PostProcessCUDA::PostProcessCUDA(const CenterPointConfig & config) : config_(config)
{
  score_thresholds_d_ = thrust::device_vector<float>(
...
cudaError_t PostProcessCUDA::generateDetectedBoxes3D_launch(
  const float * out_heatmap, const float * out_offset, const float * out_z, const float * out_dim,
  const float * out_rot, const float * out_vel, const float * out_offset_uncertainty,
  const float * out_rot_uncertainty, const float * out_vel_uncertainty,
  std::vector<Box3D> & det_boxes3d, cudaStream_t stream)
{
  ...
  generateBoxes3D_kernel<<<blocks, threads, 0, stream>>>(
    out_heatmap, out_offset, out_z, out_dim, out_rot, out_vel, out_offset_uncertainty,
    out_rot_uncertainty, out_vel_uncertainty, config_.voxel_size_x_, config_.voxel_size_y_,
    config_.range_min_x_, config_.range_min_y_, config_.down_grid_size_x_,
  ...
  const auto num_det_boxes3d = thrust::count_if(
    ...
    is_score_greater(thrust::raw_pointer_cast(score_thresholds_d_.data())));
