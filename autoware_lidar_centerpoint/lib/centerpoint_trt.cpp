...
: config_(config)
{
  ...
  std::vector<std::size_t> out_channel_sizes = {
    config_.class_size_,           config_.head_out_offset_size_,
    config_.head_out_z_size_,      config_.head_out_dim_size_,
    config_.head_out_rot_size_,    config_.head_out_vel_size_,
    config_.head_out_offset_size_, 1,
...
void CenterPointTRT::initPtr()
{
  ...
  head_out_offset_uncertainty_d_ =
    cuda::make_unique<float[]>(grid_xy_size * config_.head_out_offset_size_);
  head_out_rot_uncertainty_d_ = cuda::make_unique<float[]>(grid_xy_size * 1);
  head_out_vel_uncertainty_d_ =
    cuda::make_unique<float[]>(grid_xy_size * config_.head_out_vel_size_);
...
void CenterPointTRT::inference()
{
  ...
  std::vector<void *> head_buffers = {
    ...
    head_out_rot_d_.get(),
    head_out_vel_d_.get(),
    head_out_offset_uncertainty_d_.get(),
    head_out_rot_uncertainty_d_.get(),
    head_out_vel_uncertainty_d_.get()};
...
void CenterPointTRT::postProcess(std::vector<Box3D> & det_boxes3d)
{
  CHECK_CUDA_ERROR(post_proc_ptr_->generateDetectedBoxes3D_launch(
    ...
    head_out_rot_d_.get(), head_out_vel_d_.get(), head_out_offset_uncertainty_d_.get(),
    head_out_rot_uncertainty_d_.get(), head_out_vel_uncertainty_d_.get(), det_boxes3d, stream_));
